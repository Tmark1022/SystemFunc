...
void sig_time_out(evutil_socket_t fd, short what, void * arg)
{
...
int main(int argc, char *argv[]) {		
	...
	struct event * sigEv = evsignal_new(base, SIGINT, sigint, base); 
	evsignal_add(sigEv, NULL);
	struct event * timeEv = evtimer_new(base, sig_time_out, event_self_cbarg());	
	struct timeval tv = {5, 0};
	evtimer_add(timeEv, &tv);
